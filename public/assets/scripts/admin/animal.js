...
import { AnimalTable } from "./table/animalTable.js";

// DOM SECTION
const add = document.getElementById("animal-add");
const tbody = document.getElementById("animal-tbody");
const dialogConfirm = document.querySelector(".dialog--confirm");
const dashboardContent = document.getElementById("dashboard-content");
// SEARCH
const searchInput = document.getElementById("animal-search");
const submitButton = document.getElementById("animal-submit");
...
async function getData() {
  ...
  try {
    let newResponse = await table.fetchData("/api/animals", "POST", {
...
function openNew() {
  ...
  detailPanel.innerHTML = table.createNewContent(response.habitat);
  ...
  const habitatInput = document.getElementById("habitat-select");
  ...
  create.addEventListener("click", async (event) => {
    if (form.checkValidity()) {
      ...
      await table
        ...
        .then(() => {
          new FlashMessage("success", "l'animal à été crée");
          detailPanel.remove();
 ...
 */
async function openDetails(animal) {
  document.body.classList.add("no-scroll");

  const images = await table.fetchData("/api/animals/images", "POST", {
    id: animal.id,
  });
  ...
  updateButton.addEventListener("click", async (event) => {
    if (form.checkValidity()) {
      ...
      await table
        .fetchData("/api/animals/update", "POST", {
          id: animal.id,
        ...
        .then(() => {
          new FlashMessage("success", `l'animal ${animal.name} à été modifié`);
  ...
  deleteButton.addEventListener("click", () => {
    table.showDeleteDialog(
      dialogConfirm,
      "Supprimer l'animal ?",
      "Voulez-vous vraiment supprimer l'animal ?",
      async () =>
        await table
          .fetchData("/api/animals", "DELETE", { id: animal.id })
          .then(() => {
            new FlashMessage("success", "l'animal à bien été supprimé");
            detailPanel.remove();
            document.body.classList.remove("no-scroll");
            resetData();
          })
    ...
    .addEventListener("click", async (event) => {
      try {
        ...
        const data = await table.uploadImage(
          "api/animals/uploadImage",
          animal.id,
          fileInput.files[0]
        );

        new FlashMessage("success", `image ajoutée pour ${animal.name}`);
        ...
        new FlashMessage(
          "error",
          `impossible d'ajouter pour ${animal.name} : ${error}`
        );
      }
    });

  // when has image listening image delete button
  if (images) {
    listeningButtonsDeleteImage();
  }
}
  ...
  imageButton.forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      table.showDeleteDialog(
        ...
        async () =>
          await table
            .fetchData("/api/animals/images", "DELETE", { id: imageID })
            .then(() => {
