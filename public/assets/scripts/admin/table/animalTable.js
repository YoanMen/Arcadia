...
export class AnimalTable extends Table {
  createNewContent(habitats) {
    return `<div class="details__container details__container--show">
              <div class="details__top">
                <h3 class=" hidden--mobile">Créer un nouvelle animal</h3>
              ...
              <form id='createForm'>
                <ul>
                  ...
                  <li class="details__item">
                    ...
                    <input required minlength="3" maxlength="60" class="details__input" type="text" id="race"" >
                  </li>
                  <li class="details__item">
  ...
  createDetailContent(data, initImages, habitats) {
    return `<div class="details__container details__container--show">
              <div class="details__top">
              ...
              <form id='updateForm'>
                <ul>
                  <li class="details__item">
                    <label for='name'>nom</label>
                    <input required class="details__input" type="text" id="name"" value="${
                      data.name
                    }">
                  </li>
                  <li class="details__item">
                    <label for='race'>race</label>
                    <input required class="details__input" type="text" id="race"" value="${
                      data.race
                    }">
                  </li>
                  <li class="details__item">
                  <label for='habitat-select'>habitat</label>
                    <select id="habitat-select">
                      ${setHabitatOption(habitats, data.habitat)}
                    </select>
                  </li>
                  <li class="details__item">
  ...
  createRow(data) {
    return `<tr>
              ...
              <td class="hidden--mobile" >${data.habitat}</td>
              <td><button data-animal-id="${data.id}" class="table__button">voir</button></td>
            </tr>`;
  }

  async create(name, race, habitat, file) {
    try {
...
function setHabitatOption(habitats, data = "") {
  let content = "";

  habitats.forEach((habitat) => {
    const selected = data == habitat.name ? "selected" : "";
    content += `<option ${selected}   value="${habitat.id}">${habitat.name}</option>`;
  });
